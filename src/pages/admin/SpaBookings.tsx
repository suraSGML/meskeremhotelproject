...
type SpaBooking = Tables<'spa_bookings'> & {
  spa_services?: { name: string } | null;
};

const AdminSpaBookings = () => {
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-display text-3xl text-foreground">Spa Bookings</h1>
            ...
            {isLoading ? (
              ...
              bookings.map((booking) => (
                <TableRow key={booking.id}>
                  ...
                  <TableCell>{booking.spa_services?.name || 'N/A'}</TableCell>
                  <TableCell>{format(new Date(booking.booking_date), 'MMM d, yyyy')}</TableCell>
                  <TableCell>{booking.booking_time}</TableCell>
                  <TableCell>ETB {booking.total_amount}</TableCell>
...
export default AdminSpaBookings;
